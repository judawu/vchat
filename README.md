...
| **commonAPI.php**       | 通用工具函数库：<br>- 数据库操作（CRUD）封装函数<br>- HTTP请求处理（如curl封装）<br>- 字符串处理（加密、格式化）<br>- 主要是一个访问各家AI 公司API的通用函数 |
| **wechatMsgCrypt.php**  | 微信消息加密组件：<br>- 实现微信服务器消息加解密协议（AES-256 CBC模式）<br>- 处理微信消息的签名验证<br> |
